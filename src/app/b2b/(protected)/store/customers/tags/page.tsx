...
export default function CustomerTagsPage() {
  ...
  // Debounced value actually sent to the API (search is server-side).
  const appliedSearch = useDebouncedValue(search, 300);
  // Counts aggregated server-side over ALL active tags.
  ...
  async function fetchTags() {
    ...
    try {
      ...
      const res = await fetch(`/api/b2b/customer-tags?${params.toString()}`);
      if (res.ok) {
  ...
  async function handleDeleteTag(e: React.MouseEvent, tag: CustomerTag) {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm(`Delete tag "${tag.full_tag}"? This cannot be undone.`))
      return;
  ...
  // Group tags by prefix (the API already applied the search server-side).
  ...
  const allPrefixes = useMemo(
    () => [
      ...
      ...[...tagsByPrefix.keys()].filter(
        (p) => !TAG_PREFIXES.includes(p as TagPrefix),
      ),
    ],
    [tagsByPrefix],
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-lg bg-card shadow-sm p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">
            {t("pages.store.customerTags.totalTags")}
          </p>
        ...
        <div className="rounded-lg bg-card shadow-sm p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">
            {t("pages.store.customerTags.prefixes")}
          </p>
          <p className="text-2xl font-bold text-foreground mt-1">
            {stats.prefixCount}
          </p>
        ...
        <div className="rounded-lg bg-card shadow-sm p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">
            {t("pages.store.customerTags.totalAssignments")}
          </p>
          <p className="text-2xl font-bold text-foreground mt-1">
      ...
      {isLoading && (
        <div className="text-center py-12 text-muted-foreground">
          {t("pages.store.customerTags.loadingTags")}
        </div>
      )}

      {/* Empty (no tags defined at all) */}
      {!isLoading && tags.length === 0 && !appliedSearch && (
        <div className="text-center py-12 border border-dashed border-border rounded-lg">
          <Tag className="h-8 w-8 mx-auto text-muted-foreground mb-3" />
          <p className="text-muted-foreground">
            {t("pages.store.customerTags.noTagsDefined")}
          </p>
          <button
            onClick={() => setShowCreateModal(true)}
      ...
      {/* No search results (the API returned nothing for the active search) */}
      {!isLoading && tags.length === 0 && appliedSearch && (
            ...
            <div key={prefix} className="rounded-lg bg-card shadow-sm">
              <div className="p-4 border-b border-border">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t("pages.store.customerTags.prefix")}:{" "}
