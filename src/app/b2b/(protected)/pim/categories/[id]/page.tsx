...
export default function CategoryDetailPage() {
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      const res = await fetch(`/api/b2b/pim/categories/${categoryId}/products?${params}`);
  ...
  async function handleBulkAssociate(entityCodes: string[]) {
    try {
      const res = await fetch(`/api/b2b/pim/categories/${categoryId}/products`, {
        method: "POST",
  ...
  async function handleBulkDisassociate() {
    ...
    if (!confirm(`Remove ${selectedProducts.size} product(s) from this category?`)) {
      return;
    }
  ...
  async function handleExport(format: "csv" | "xlsx" | "txt") {
    try {
      const res = await fetch(`/api/b2b/pim/categories/${categoryId}/export?format=${format}`);
  ...
  async function handleImport() {
    ...
    try {
      if (importMethod === "file") {
        ...
        const res = await fetch(`/api/b2b/pim/categories/${categoryId}/import?action=${importAction}`, {
          method: "POST",
          body: formData,
        });
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    ...
                    {typeof category.child_count === "number" && (
                      <span>Children: {category.child_count}</span>
                    )}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-3xl rounded-xl border border-border bg-card shadow-xl">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <div>
                <h2 className="text-lg font-semibold text-foreground">Add products to category</h2>
                <p className="text-sm text-muted-foreground">
                  Select products to associate with {category.name}
                </p>
              </div>
              <button
          ...
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card shadow-xl">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Upload a file to add or remove products from {category.name}
                </p>
              </div>
              <button
            ...
            <div className="space-y-6 px-6 py-6">
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
                Use the template exported from this category to ensure the correct columns are
