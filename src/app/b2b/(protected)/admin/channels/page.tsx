  ...
  return (
    <div className="p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("pages.admin.channels.title")}</h1>
      ...
      {error && (
        <div className="mb-4 p-4 bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400 rounded-lg">{error}</div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-card rounded-xl border border-border">
          ...
          <p className="text-muted-foreground mb-4">{t("pages.admin.channels.noChannels")}</p>
          <button
            onClick={openCreate}
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors"
          >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted">
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("pages.admin.channels.channel")}</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("pages.admin.channels.name")}</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("pages.admin.channels.default")}</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("pages.admin.channels.status")}</th>
                <th className="text-right px-4 py-3 font-medium text-muted-foreground">{t("common.actions")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {channels.map((channel) => (
                <tr key={channel.code} className="hover:bg-muted/50">
                  ...
                  <td className="px-4 py-3">
                    {channel.is_default && (
                      <span className="inline-flex items-center gap-1 text-xs bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 px-2 py-0.5 rounded-full">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {!channel.is_default && (
                        <button
                          ...
                          className="p-1.5 text-muted-foreground hover:text-rose-600 rounded transition-colors disabled:opacity-50"
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-md">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:bg-muted disabled:text-muted-foreground font-mono"
            />
            ...
            <div className="flex flex-wrap gap-2">
              {COLOR_SWATCHES.map((swatch) => (
                <button
                  ...
                  } ${!swatch.hex ? "bg-muted border-border" : ""}`}
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-foreground">{t("pages.admin.channels.defaultChannel")}</span>
            ...
            {isEditing && (
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <div>
                  <span className="text-sm font-medium text-foreground">{t("pages.admin.channels.activeLabel")}</span>
                  <p className="text-xs text-muted-foreground">
          ...
          {error && (
            <div className="p-3 bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400 text-sm rounded-lg">{error}</div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-sm rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
