...
export async function GET(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: { versions: config.versions, currentVersion: config.currentVersion, currentPublishedVersion: config.currentPublishedVersion } });
  } catch (error) {
    return blogError(error, "Failed to list versions");
  }
}
