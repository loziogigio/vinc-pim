...
import { getB2BPageTemplateConfig } from "@/lib/db/b2b-page-templates";
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const config = await getB2BPageTemplateConfig(slug, pageSlug, auth.tenantDb);

    return NextResponse.json(config);
  } catch (error) {
    console.error("[GET /api/b2b/b2b/portals/[slug]/pages/[pageSlug]/template]", error);
