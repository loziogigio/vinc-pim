...
import {
  getPublishedB2BPageTemplate,
  getLatestB2BPageTemplate,
 ...
 *
 * Returns published page content (blocks, seo) for a B2B portal frontend.
 * Auth: API key + Origin header → portal lookup by domain.
 *
 * Query params:
 *   preview=true  Return the latest draft instead of the published version
 *
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 3. Fetch the page template (published or latest draft)
    const isPreview = req.nextUrl.searchParams.get("preview") === "true";
    const template = isPreview
      ? await getLatestB2BPageTemplate(portal.slug, pageSlug, tenantDb)
      : await getPublishedB2BPageTemplate(portal.slug, pageSlug, tenantDb);
    ...
    console.error("[GET /api/b2b/b2b/public/pages/[pageSlug]]", error);
