...
import { loadBlogContentVersion } from "@/lib/services/blog/blog-content.service";
...
/** POST — make a version current for a post in this storefront. */
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const config = await loadBlogContentVersion(ctx.tenantDb, postId, locale, version);
    return NextResponse.json(config);
  } catch (error) {
    return blogError(error, "Failed to load version");
  }
}
