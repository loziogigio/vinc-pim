/**
 * POST /api/b2b/reminders/status/bulk — Bulk reminder status check
 */

import { NextRequest, NextResponse } from "next/server";
import { requireTenantAuth } from "@/lib/auth/tenant-auth";
import { getBulkReminderStatus } from "@/lib/services/reminder.service";
...
export async function POST(req: NextRequest) {
  ...
  const result = await getBulkReminderStatus(tenantDb, tenantId, userId!, body.skus);
