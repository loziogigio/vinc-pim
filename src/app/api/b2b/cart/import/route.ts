 ...
 *   merge_mode?: "skip" | "error" | "replace" | "merge",   // default "skip"
 ...
 * - Idempotency: keyed on `erp_cart_id === external_cart_id`. When a cart
 *   with that key already exists, behaviour depends on merge_mode:
 *     - "skip" (default): `{ status: "skipped", reason: "already_exists" }`
 *     - "error": `{ status: "failed", ... }`
 *     - "replace": the existing draft becomes the rebuilt payload — items,
 *        totals and delivery info are overwritten.
 *     - "merge": the rebuilt payload is folded into the existing draft —
 *        for a SKU present in both, the incoming (ERP) line wins (qty +
 *        price); SKUs the incoming cart doesn't know about are kept;
 *        incoming-only SKUs are added. Auto-generated promo gift lines on
 *        the existing cart are dropped and regenerated by saveOrder().
 *   For replace/merge: order_id / _id / cart_number / year / created_at /
 *   session_id / flow_id are preserved, so anything referencing the cart
 *   stays valid. Returns `{ status: "updated" }`, or `{ status: "skipped",
 *   reason: "unchanged" }` when the resulting cart already matches what's
 *   stored (a re-run whose only effect would be to bump __v) — so merge is
 *   idempotent and safe on a cron.
...
interface CartImportRequest {
  source_id: string;
  batch_id?: string;
  merge_mode?: "skip" | "error" | "replace" | "merge";
  carts: CartImportItem[];
}
...
async function importSingleCart(
  ...
  mergeMode: "skip" | "error" | "replace" | "merge",
  tenantId: string,
  tenantDb: string,
  ...
  // Under merge_mode=replace/merge an existing cart isn't a dead end — we
  // patch it in place further down (after the line items are rebuilt).
  // skip/error keep the original short-circuit behaviour.
  const isReplace = !!existingByErp && mergeMode === "replace";
  const isMerge = !!existingByErp && mergeMode === "merge";

  if (existingByErp && !isReplace && !isMerge) {
    if (mergeMode === "skip") {
      return {
        external_cart_id: cart.external_cart_id,
        status: "skipped",
  ...
  //    Skipped on replace/merge — the target cart already exists and is
  //    patched in place; re-running the parking dance would just park (and
  //    then un-park) the very cart we're updating.
  ...
  if (!isReplace && !isMerge && isCurrent && parkExisting) {
  ...
  // 4b. Replace / merge: patch the existing draft in place and return.
  //     replace = the rebuilt payload becomes the cart; merge = the rebuilt
  //     payload is folded in (ERP wins for SKUs in both, lines the ERP cart
  //     doesn't know about are kept). Either way order_id / _id /
  //     cart_number / year / created_at / session_id / flow_id are kept;
  //     only contents, totals, delivery metadata and current-flag move.
  if ((isReplace || isMerge) && existingByErp) {
    return updateExistingCart(
      existingByErp.order_id,
      cart,
      batch,
      tenantId,
      OrderModel,
...
// ─────────────────────────────────────────────────────────────────────
// merge_mode=replace — patch an existing cart in place
// ─────────────────────────────────────────────────────────────────────

/**
 * Two cart line lists are "equal" for replace purposes if they hold the
 * same SKUs with the same quantities, prices, VAT and promo pricing —
 * line order and bookkeeping fields (added_at, line_number, …) are ignored.
 * Used to no-op a replace whose only effect would be to bump __v / updated_at.
 */
function lineItemsEqual(
  ...
  const norm = (items: any[]) =>
    items
      .map((i) => ({
        e: String(i.entity_code ?? ""),
        s: String(i.sku ?? ""),
        q: Number(i.quantity ?? 0),
        l: Number(i.list_price ?? 0),
        u: Number(i.unit_price ?? 0),
        v: Number(i.vat_rate ?? 0),
        p: Number(i.promo_price ?? 0),
        pc: String(i.promo_code ?? ""),
      }))
      .sort((x, y) => `${x.e}|${x.s}`.localeCompare(`${y.e}|${y.s}`));
...
async function updateExistingCart(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  address: any,
  // The freshly-rebuilt lines from the payload (the ERP cart).
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  incomingLineItems: any[],
  ...
  if (!existing) {
    // Vanished between the idempotency read and now — let the caller retry
    // (a follow-up run will create it fresh).
    return {
      external_cart_id: cart.external_cart_id,
      status: "failed",
      reason: `existing cart disappeared before ${mode}`,
    };
  }

  // Compute the final line list.
  //   replace → the incoming (ERP) lines, full stop.
  //   merge   → incoming lines, plus any line on the existing cart whose SKU
  //             the incoming cart doesn't carry. Auto-generated promo gift
  //             lines are dropped — saveOrder()'s enforcePromoGifts will
  //             regenerate them from the merged cart's promo progress.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let finalItems: any[] = incomingLineItems;
  if (mode === "merge") {
    const incomingCodes = new Set(
      incomingLineItems.map((i) => String(i.entity_code ?? "")),
    );
    const carried = (existing.items ?? [])
      ...
      .filter(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (i: any) =>
          !i.is_gift_line && !incomingCodes.has(String(i.entity_code ?? "")),
      )
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((i: any) =>
        typeof i.toObject === "function" ? i.toObject() : { ...i },
      );
    finalItems = [...incomingLineItems, ...carried];
    // Re-number 10, 20, 30, … so line_number stays the contiguous sequence
    // the rest of the order code expects.
    finalItems.forEach((it, idx) => {
      it.line_number = (idx + 1) * 10;
      it.updated_at = new Date();
    });
  }

  // No-op short-circuit: if the resulting cart matches what's already stored
  // (same lines, same current flag) there's nothing to do — important for
  // cron use, where any legacy-side touch wipes imported_at and re-queues
  // the cart even when its contents didn't change.
  if (
    existing.is_current === isCurrent &&
    lineItemsEqual(existing.items ?? [], finalItems)
  ) {
    return {
      external_cart_id: cart.external_cart_id,
      status: "skipped",
  ...
  // Roll up the order totals from the final line list (the per-line
  // line_gross/net/vat/total were computed when each line was built;
  // saveOrder() recomputes these anyway, but keep them consistent here too).
  let subtotalGross = 0;
  let subtotalNet = 0;
  let totalVat = 0;
  let orderTotal = 0;
  for (const it of finalItems) {
    subtotalGross += Number(it.line_gross ?? 0);
    subtotalNet += Number(it.line_net ?? 0);
    totalVat += Number(it.line_vat ?? 0);
    orderTotal += Number(it.line_total ?? 0);
  }

  existing.items = finalItems;
  existing.price_decimals = priceDecimals;
  existing.subtotal_gross = round2(subtotalGross);
  existing.subtotal_net = round2(subtotalNet);
  existing.total_discount = 0;
  existing.total_vat = round2(totalVat);
  existing.order_total = round2(orderTotal);
  existing.is_current = isCurrent;
  existing.shipping_address_id = address.address_id;
  existing.shipping_address_code = address.external_code;
  existing.effective_tags = resolveEffectiveTags(customer, address);
  if (cart.pricelist_type !== undefined) {
    existing.pricelist_type = cart.pricelist_type;
  }
  if (cart.pricelist_code !== undefined) {
    existing.pricelist_code = cart.pricelist_code;
  }
  if (cart.notes !== undefined) existing.notes = cart.notes;
  if (cart.requested_delivery_date) {
    existing.requested_delivery_date = new Date(cart.requested_delivery_date);
  }
  if (cart.delivery_slot) existing.delivery_slot = cart.delivery_slot;
  if (cart.delivery_route) existing.delivery_route = cart.delivery_route;
  // Active carts have no name; parked carts keep the label the caller sends
  // (falling back to whatever was there).
  existing.cart_name = isCurrent
    ? undefined
    : cart.cart_name ?? existing.cart_name;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const prevErp: Record<string, any> =
    existing.erp_data && typeof existing.erp_data.toObject === "function"
      ? existing.erp_data.toObject()
      : { ...(existing.erp_data ?? {}) };
  existing.erp_data = {
    ...prevErp,
    erp_cart_id: cart.external_cart_id,
    legacy_source: cart.legacy_source ?? batch.source_id ?? prevErp.legacy_source,
    legacy_batch_id: batch.batch_id ?? prevErp.legacy_batch_id,
    legacy_imported_at: new Date().toISOString(),
    legacy_created_at: cart.created_at ?? prevErp.legacy_created_at,
    ...(cart.delivery_info
      ? { delivery_info: cart.delivery_info }
      : prevErp.delivery_info
        ? { delivery_info: prevErp.delivery_info }
        : {}),
  };
  existing.markModified("erp_data");

  try {
    // Same enrichment pipeline the create path runs (totals roll-up, promo
    // progress, gift enforcement) — then persists.
    await saveOrder(existing);
  } catch (saveErr) {
    console.error(
      `[cart/import] saveOrder (post-${mode}) failed for`,
      cart.external_cart_id,
      saveErr,
    );
    return {
