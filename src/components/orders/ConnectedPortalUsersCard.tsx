...
import { UserCog, CheckCircle, XCircle, Eye, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="rounded-lg bg-card shadow-sm">
      <button
        ...
        className="w-full p-4 flex items-center justify-between cursor-pointer hover:bg-muted/30 transition rounded-lg"
      >
        <h2 className="font-semibold text-foreground flex items-center gap-2">
          <UserCog className="h-4 w-4 text-blue-600" />
          {t("pages.store.customerDetail.connectedPortalUsers")}
          {!isLoading && portalUsers.length > 0 && (
      ...
      {isOpen && (
        <>
          {isLoading ? (
            <div className="px-4 pb-4 text-sm text-muted-foreground">{t("common.loading")}</div>
