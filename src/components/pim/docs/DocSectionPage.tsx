...
function MethodBadge({ method }: { method: string }) {
  ...
  return <span className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold ${c[method] || "bg-muted text-muted-foreground"}`}>{method}</span>;
}
  ...
  if (!sectionDef) {
    return (
      <div className="text-center py-20 text-muted-foreground">
        ...
        <Link href={`${tenantPrefix}/b2b/pim/documentation`} className="mt-4 inline-flex items-center gap-1 text-sm text-primary hover:underline">
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[0.428rem] border border-border bg-card p-6 shadow-[0_4px_24px_0_rgba(34,41,47,0.08)]">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-foreground">{t(sectionDef.titleKey)}</h1>
          {sectionDef.pimHref && (
            <Link
              href={`${tenantPrefix}${sectionDef.pimHref}`}
              className="ml-auto flex items-center gap-1 rounded-[0.358rem] border border-border px-3 py-1.5 text-xs font-medium text-primary transition hover:bg-primary/10"
            >
        ...
        {/* Description */}
        <p className="mb-4 text-sm leading-relaxed text-muted-foreground">{t(sectionDef.descKey)}</p>

        {/* Standard content paragraphs */}
        {!isSearchFilters && !isApiRef && contentKeys.map((key) => (
          <p key={key} className="mb-3 text-sm leading-relaxed text-muted-foreground">{t(key)}</p>
        ))}
          ...
          <div className="space-y-5">
            {SEARCH_SUB_SECTIONS.map((sub) => (
              <div key={sub.titleKey}>
                ...
                <p className="mb-2 pl-6 text-sm leading-relaxed text-muted-foreground">{t(sub.descKey)}</p>
                {sub.items && (
...
function ApiReferenceContent({ t }: { t: (key: string) => string }) {
  return (
    <div className="space-y-8">
      {/* Auth */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiAuthTitle")}</h3>
      ...
      {/* Endpoints table */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiEndpointsTitle")}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border"><th className="px-3 py-2 text-left text-xs font-semibold text-foreground">Method</th><th className="px-3 py-2 text-left text-xs font-semibold text-foreground">Endpoint</th><th className="px-3 py-2 text-left text-xs font-semibold text-foreground">Description</th></tr></thead>
            <tbody>
              {API_SUBSECTIONS.endpoints.map((ep) => (
                <tr key={`${ep.method}-${ep.path}`} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">{t(ep.key)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Filter params */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiFiltersTitle")}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border"><th className="px-3 py-2 text-left text-xs font-semibold text-foreground">Parameter</th><th className="px-3 py-2 text-left text-xs font-semibold text-foreground">Description</th></tr></thead>
            <tbody>
              {API_SUBSECTIONS.filterParams.map((fp) => (
                <tr key={fp.param} className="border-b border-border last:border-0">
                  <td className="px-3 py-2 font-mono text-xs font-semibold text-primary">{fp.param}</td>
                  <td className="px-3 py-2 text-muted-foreground">{t(fp.key)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Updatable fields */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiUpdateTitle")}</h3>
        <p className="mb-2 text-sm text-muted-foreground">{t("pages.pim.documentation.apiUpdateDesc")}</p>
        <ul className="space-y-1.5">{API_SUBSECTIONS.updateGroups.map((k) => <Bullet key={k}>{t(k)}</Bullet>)}</ul>
      </div>

      {/* Other endpoints */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiOtherTitle")}</h3>
      ...
      {/* Response format */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">{t("pages.pim.documentation.apiResponseTitle")}</h3>
        <p className="mb-3 text-sm text-muted-foreground">{t("pages.pim.documentation.apiResponseDesc")}</p>
