    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-card shadow-2xl">
        ...
        <div className="flex items-center gap-4 border-b border-border p-6 sticky top-0 bg-card z-10">
          <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-amber-100 dark:bg-amber-500/15">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-foreground">Pricing Method</h4>
              {listPrice > 0 ? (
                <div className="text-xs text-right">
                  {hasSalePrice ? (
                    <>
                      <span className="text-muted-foreground line-through">€{listPrice.toFixed(2)}</span>
                      <span className="ml-2 text-emerald-600 font-medium dark:text-emerald-400">Sale: €{salePrice!.toFixed(2)}</span>
                    </>
                  ) : (
                    <span className="text-muted-foreground">List: €{listPrice.toFixed(2)}</span>
                  )}
                </div>
              ) : (
                <div className="text-xs text-amber-600 dark:text-amber-400">No list price set</div>
              )}
            </div>
                ...
                <div>
                  ...
                  {!hasMultipleTargets && formData.discount_percentage && formData.discount_percentage > 0 && listPrice > 0 && (
                    <div className="mt-4 pt-4 border-t border-border">
                      ...
                      <div className="text-xs text-muted-foreground mb-3 font-mono bg-card rounded p-2 border border-border">
                        €{listPrice.toFixed(2)}
                        {hasSalePrice && (
                          <> <span className="text-amber-600 dark:text-amber-400">→ -{Math.round((1 - salePrice! / listPrice) * 100)}% sale</span> → €{salePrice!.toFixed(2)}</>
                        )}
                        <span className="text-amber-600 dark:text-amber-400"> → -{formData.discount_percentage}% promo</span>
                        <span className="text-emerald-700 dark:text-emerald-400 font-bold"> = €{(formData.promo_price || 0).toFixed(2)}</span>
                      </div>
                      <div className="flex items-end justify-between">
                        <div>
                          <div className="text-xs text-emerald-600 dark:text-emerald-400 mb-1">Promo Unit Price</div>
                          <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-400">€{(formData.promo_price || 0).toFixed(2)}</div>
                        </div>
                        {selectedPackaging && selectedPackaging.qty > 1 && formData.promo_price && (
                          <div className="text-right">
                            <div className="text-xs text-muted-foreground mb-1">Package ({selectedPackaging.code} × {selectedPackaging.qty})</div>
                            <div className="text-lg font-semibold text-foreground">€{(formData.promo_price * selectedPackaging.qty).toFixed(2)}</div>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {!hasMultipleTargets && formData.discount_amount && formData.discount_amount > 0 && listPrice > 0 && (
                    <div className="mt-4 pt-4 border-t border-border">
                      ...
                      <div className="text-xs text-muted-foreground mb-3 font-mono bg-card rounded p-2 border border-border">
                        ...
                        <span className="text-amber-600 dark:text-amber-400"> → -€{formData.discount_amount.toFixed(2)} promo</span>
                        <span className="text-emerald-700 dark:text-emerald-400 font-bold"> = €{(formData.promo_price || 0).toFixed(2)}</span>
                      </div>
                      <div className="flex items-end justify-between">
                        <div>
              ...
              {pricingMethod === "direct" && (
                <div>
                  ...
                  {!hasMultipleTargets && formData.promo_price && formData.promo_price > 0 && (
                    <div className="mt-4 pt-4 border-t border-border">
                      <div className="text-xs text-muted-foreground mb-2 font-mono bg-card rounded p-2 border border-border">
                        Net price (no discount calculation)
                      </div>
                      <div className="flex items-end justify-between">
                        <div>
                          <div className="text-xs text-emerald-600 dark:text-emerald-400 mb-1">Promo Unit Price</div>
                          <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-400">€{formData.promo_price.toFixed(2)}</div>
                        </div>
          ...
          {(() => {
            ...
            return (
              <div className="bg-muted rounded-lg p-4 -mt-2">
                <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Per-Packaging Preview</h4>
                <div className="space-y-1.5">
                  {targets.map((pkg) => {
                    ...
                    return (
                      <div key={pkg.pkg_id} className="flex items-center justify-between text-xs bg-card rounded px-3 py-2 border border-border">
                        ...
                        <span className="flex-shrink-0 font-mono">
                          {pkgBase > 0 && (
                            <>
                              <span className="text-muted-foreground">€{pkgBase.toFixed(2)}</span>
                              {pricingMethod === "percentage" && formData.discount_percentage && (
                                <span className="text-amber-600 dark:text-amber-400 mx-1">-{formData.discount_percentage}%</span>
                              )}
                              {pricingMethod === "amount" && formData.discount_amount && (
                                <span className="text-amber-600 dark:text-amber-400 mx-1">-€{formData.discount_amount.toFixed(2)}</span>
                              )}
                              {pricingMethod !== "direct" && <span className="text-muted-foreground mx-1">=</span>}
                            </>
                          )}
          ...
          {/* Conditions */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-semibold text-foreground mb-3">Conditions</h4>
          ...
          {/* Dates */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-semibold text-foreground mb-3">Validity Period</h4>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">
                  Start Date
          ...
          <div className="flex gap-6">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded border-border text-amber-600 focus:ring-amber-500"
              />
              <span className="text-sm text-foreground">Stackable with other promotions</span>
            </label>
          </div>
