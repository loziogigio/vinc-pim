/**
 * Seed: coupon_settings data model (channel-scoped MyMB coupon backend config)
 *
 ...
 * The record holds only `enabled` + `api_url`; the MyMB Basic-auth credentials
 * live in the storefront env (COUPON_API_USER / COUPON_API_PASSWORD), never here.
 *
 ...
 * /b2b/admin/data-models/coupon_settings.
 *
 * Usage:
 *   pnpm tsx src/scripts/seed-data-model-coupon-settings.ts --tenant <id> --channel b2b --api-url <url>
 *   pnpm tsx src/scripts/seed-data-model-coupon-settings.ts --tenant <id> --channel b2b --api-url <url> --dry-run
 *   pnpm tsx src/scripts/seed-data-model-coupon-settings.ts --tenant <id> --channel b2b --api-url <url> --force
 *   pnpm tsx src/scripts/seed-data-model-coupon-settings.ts --tenant <id> --channel b2b --disabled
 *
 * `--api-url` is the MyMB coupon service base URL (no credentials). When given,
 * the record is seeded `enabled: true`; pass `--disabled` to force it off.
 */
...
import { COUPON_SETTINGS_BLUEPRINT } from "@/lib/data-models/blueprints/coupon-settings";
...
const { definition: BLUEPRINT_DEF } = COUPON_SETTINGS_BLUEPRINT;
...
async function main() {
  ...
  if (!args.tenant) {
    console.error(
      "Usage: --tenant <id> [--channel <code>] [--api-url <url>] [--disabled] [--dry-run] [--force]"
    );
    process.exit(1);
  }
  ...
  console.log(`\n📋 Seed coupon_settings data model`);
