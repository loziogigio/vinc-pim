 ...
 * B2B Sitemap Model
 *
 * Stores per-portal sitemap data: collected URLs, robots.txt config,
 * generation stats, and validation results.
 *
 * The actual sitemap.xml and robots.txt are generated on the B2B portal
 * side — this model stores the structured data they need.
 *
 * Collection: b2bsitemaps
 */
...
export interface IB2BSitemap {
  _id?: string;
  /** Which portal this belongs to (unique) */
  portal_slug: string;
...
const B2BSitemapSchema = new Schema(
  {
    portal_slug: {
      type: String,
      required: true,
...
B2BSitemapSchema.index({ portal_slug: 1 }, { unique: true });

export { B2BSitemapSchema };
