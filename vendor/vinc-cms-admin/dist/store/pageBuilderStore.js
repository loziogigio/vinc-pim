...
exports.usePageBuilderStore = void 0;
const zustand_1 = require("zustand");
const nanoid_1 = require("nanoid");
const registry_js_1 = require("../registry.js");
const cloneBlocks = (blocks) => JSON.parse(JSON.stringify(blocks));
exports.usePageBuilderStore = (0, zustand_1.create)((set, get) => ({
    blocks: [],
    ...
    loadPageConfig: (config) => {
        ...
        const isProductDetailPage = config.slug === "product-detail" ||
        ...
        const normalizedBlocks = [...blocks]
            ...
            const normalized = {
                ...
                ...(isProductDetailPage && !hasZone ? { zone: "zone1" } : {})
            };
    ...
    addBlock: (variantId) => {
        const template = (0, registry_js_1.getBlockTemplate)(variantId);
        if (!template)
            return;
        const slug = get().pageDetails.slug;
        const isProductDetailPage = slug === "product-detail" ||
            slug.startsWith("product-detail-") ||
            slug.startsWith("sku-") ||
            slug.startsWith("parentSku-");
        const defaultZone = variantId === "product-data-table" ? "zone1" : "zone3";
        const newBlock = {
    ...
    updateBlockConfig: (blockId, config) => {
        console.log('[Store] updateBlockConfig called, blockId:', blockId, 'config title:', config?.title);
        set((state) => {
            const updatedBlocks = state.blocks.map((block) => block.id === blockId ? { ...block, config: { ...block.config, ...config } } : block);
            const updatedBlock = updatedBlocks.find(b => b.id === blockId);
            console.log('[Store] After update, block title:', updatedBlock?.config?.title);
            return {
    ...
    getPagePayload: () => {
        ...
        carouselBlocks.forEach((b, i) => {
            console.log(`[getPagePayload] carousel-products[${i}] title:`, b.config?.title);
        });
        const payload = {
...
//# sourceMappingURL=pageBuilderStore.js.map